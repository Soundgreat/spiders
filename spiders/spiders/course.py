...
class CourseSpider(scrapy.Spider):
    ...
    def parse_details(self, response):
        ...
        try:
            scoreAverageInt = int(response.css('div.course-score span.course-score-average::text').extract_first()[:-1])
            scoreAverageFloat = int(response.css('div.course-score span.course-score-average small::text').extract_first())
            scoreAverage = scoreAverageInt + scoreAverageFloat / 10
            item['scoreAverage'] = scoreAverage
        except:
            item['scoreAverage'] = None
        try:
            scorePeople = int(response.css('div.course-score span.course-score-people::text').extract_first()[1:-2])
            item['scorePeople'] = scorePeople
        except:
            item['scorePeople'] = None
        try:
            item['teachers'] = response.css('ul.course-info li')[-1].css('b::text').extract_first().split('，')
        except:
            item['teachers'] = None
        try:
